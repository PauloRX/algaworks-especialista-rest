...
public class UsuarioController {
	...
	@Autowired
	private UsuarioInputDesassembler desassembler;
	...
	public UsuarioModel buscar(@PathVariable Long usuarioId) {
		return assembler.toModel(cadastroUsuario.buscarOuFalhar(usuarioId));
	}
	
	@PostMapping
	@ResponseStatus(code = HttpStatus.CREATED)
	public UsuarioModel adicionar(@RequestBody @Valid UsuarioComSenhaInput input) {
		Usuario usuario = desassembler.toDomainModel(input);
	...
	public UsuarioModel atualizar(@PathVariable Long usuarioId, @RequestBody @Valid UsuarioInput input) {
		...
		desassembler.copyToObjectModel(input, usuario);
		return assembler.toModel(cadastroUsuario.salvar(usuario));
	}
	
	@PutMapping("/{usuarioId}/atualizar")
	@ResponseStatus(code = HttpStatus.NO_CONTENT)
	public void atualizarSenha(@PathVariable Long usuarioId, @RequestBody @Valid SenhaInput input) {
		cadastroUsuario.alterarSenha(usuarioId, input.getSenhaAtual(), input.getNovaSenha());
	}
